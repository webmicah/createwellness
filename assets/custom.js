(function($) { 
  ...
  html += '<li class="item-search">';
    html += '<a href="'+product.url+'">';
      html += '<div class="thumb">';
      if(product.image){
        // Add 'data-src' attribute and 'lazyload' class
        html += '<img class="lazyload" data-src="'+theme.Images.getSizedImageUrl(product.image, '240x240_crop_center')+'" alt="'+product.title+'">';
      }
      html += '</div>';
      html += '<div class="product-info">';
        html += '<h4 class="product-name">'+product.title+'</h4>';
  ...
  //debounce function to be used on ajax calls that need a delay
  ...
  // Add wishlist
$("body").on('click', '.kt-wishlist', debounce(function(event) {
  if (!Shopify.customer) {
  ...
  // Remove wishlist
$("body").on('click', '.nitro_wishlist_remove', debounce(function(event) {
  event.preventDefault();
  var $this = $(this),
      value = $(this).data('id');
